...
cross_green = expyriment.stimuli.FixCross(size=(50, 50), line_width=4, colour=expyriment.misc.constants.C_GREEN) # Detection cross
cross_green.preload()

# BLOCKS
spectral_expectation_blocks = ["fixed_spectral", "variable_spectral"]*4
random.shuffle(spectral_expectation_blocks) # 8 blocks counterbalanced

# TRIALS
temporal_expectation_trials = ["fixed_SOA","variable_SOA"]*30 # 60 trials 
...
variable_SOA_list = variable_SOA * 3
random.shuffle(variable_SOA_list)

variable_target_list = variable_target_freq * 3
...
if keys and any(k == expyriment.misc.constants.K_ESCAPE for k, t in keys):
    expyriment.control.end()
